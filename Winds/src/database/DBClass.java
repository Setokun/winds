...
public class DBClass{    
    ...
    public static void createStructures(){
    	try {
			...
			DBClass.executeQuery("CREATE TABLE users (id INT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) PRIMARY KEY, email VARCHAR, password VARCHAR, pseudo VARCHAR, registrationDate VARCHAR, userType VARCHAR, userStatus VARCHAR)");
    ...
    public static void createTestData(){
		...
		try {
			for (int i = 1; i <= 4; i++) {
				executeQuery("INSERT INTO users (email, password, pseudo, registrationDate, userType, userStatus) VALUES ('perso"+i+"@live.fr','pwd"+i+"','pseudo"+i+"','2015-05-0"+i+" 12:05:27','player','ok');");
			}
			executeQuery("INSERT INTO users (email, password, pseudo, registrationDate, userType, userStatus) VALUES ('admin@live.fr','admin','admin','2015-04-30 00:05:00','administrator','ok');");
			for (int i = 1; i <= 20; i++) {
    ...
    public static  ResultSet getScores(){
		try {
			...
			return DBClass.requestQuery("SELECT levels.name AS levelName, nbItems, nbClicks, time, rank FROM scores JOIN levels ON levels.id = scores.idLevel WHERE idPlayer="+1);
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		} catch (SQLException e) {
			e.printStackTrace();
